...
func (s *Store) Load() (*Config, error) {
	...
	for scanner.Scan() {
		...
		case "vault_path":
			cfg.VaultPath = value
		case "second_brain_path":
			cfg.SecondBrainPath = value
		case "website_path":
			cfg.WebsitePath = value
		case "website_url":
			cfg.WebsiteURL = value
		case "recall_socket":
			cfg.RecallSocket = value
		}
	}
...
func (s *Store) Save(cfg *Config) error {
	...
	if cfg.SecondBrainPath != "" {
		b.WriteString("\n")
		b.WriteString("# Path to your second brain vault\n")
		fmt.Fprintf(&b, "second_brain_path=%s\n", cfg.SecondBrainPath)
	}
	if cfg.WebsitePath != "" {
		b.WriteString("\n")
	...
	if cfg.WebsiteURL != "" {
		b.WriteString("\n")
		b.WriteString("# Base URL of your website (for obsidian sync links)\n")
		fmt.Fprintf(&b, "website_url=%s\n", cfg.WebsiteURL)
	}
...
// ResolveSecondBrainPath returns the second-brain vault path from config or environment.
// Priority: config file second_brain_path > OBSIDIAN_SECOND_BRAIN_PATH env var.
func ResolveSecondBrainPath() string {
	cfg, err := Load()
	if err == nil && cfg.SecondBrainPath != "" {
		return cfg.SecondBrainPath
	}
	return os.Getenv("OBSIDIAN_SECOND_BRAIN_PATH")
}
...
// ResolveWebsiteURL returns the website base URL from config or environment.
func ResolveWebsiteURL() string {
	cfg, err := Load()
	if err == nil && cfg.WebsiteURL != "" {
		return cfg.WebsiteURL
	}
	return os.Getenv("OBSIDIAN_WEBSITE_URL")
}

// ResolveRecallSocket returns the recall RPC socket path from config or
// environment, mirroring the daemon's resolver in
// obsidian-indexer/main.go:resolveSocketPath. Order: config file > env var
// > default (~/.alluka/run/obsidian.sock).
func ResolveRecallSocket() string {
	cfg, err := Load()
	if err == nil && cfg.RecallSocket != "" {
		return cfg.RecallSocket
	}
...
// RFC §7 Phase 0 (TRK-524): VaultPath + SecondBrainPath resolution.

// VaultPath reads vault_path from the config file and returns its absolute value.
// Relative paths are resolved against $HOME.
// Returns an error if the key is missing or the file cannot be read.
func (s *Store) VaultPath() (string, error) {
	return s.getConfigKey("vault_path")
}

// SecondBrainPath reads second_brain_path from the config file and returns its absolute value.
// Relative paths are resolved against $HOME.
// Returns an error if the key is missing or the file cannot be read.
func (s *Store) SecondBrainPath() (string, error) {
	return s.getConfigKey("second_brain_path")
}

// getConfigKey reads a single key from the config file, resolving relative paths.
func (s *Store) getConfigKey(key string) (string, error) {
	p, err := s.Path()
	if err != nil {
		return "", err
	}
	...
	for scanner.Scan() {
		...
		if strings.TrimSpace(parts[0]) == key {
			...
			return resolveRelativePath(val)
		}
	}
	if err := scanner.Err(); err != nil {
		return "", fmt.Errorf("reading config: %w", err)
	}
	return "", fmt.Errorf("config key %q missing in %s", key, p)
}

// resolveRelativePath returns val unchanged if absolute, otherwise joins with $HOME.
func resolveRelativePath(val string) (string, error) {
	...
	return filepath.Join(home, val), nil
}
