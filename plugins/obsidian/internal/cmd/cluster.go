...
func ClusterCmd(vaultPath string, opts ClusterOptions) error {
	captures, err := loadUntaggedCaptures(vaultPath, opts.Kind)
	if err != nil {
...
func loadUntaggedCaptures(vaultPath string, kinds ...vault.VaultKind) ([]*promoteNoteInfo, error) {
	...
	for _, info := range allNotes {
		...
		isCapture := strings.HasPrefix(info.Path, schema.Inbox+"/") ||
			frontmatterString(parsed.Frontmatter, "type") == vault.TypeFleeting
		if !isCapture {
			continue
		}
