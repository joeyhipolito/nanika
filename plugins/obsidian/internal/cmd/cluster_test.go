...
func TestClusterID_Stable(t *testing.T) {
	paths := []string{vault.NanikaInbox + "/b.md", vault.NanikaInbox + "/a.md", vault.NanikaInbox + "/c.md"}
	id1 := clusterID(paths)
	id2 := clusterID(paths)
	if id1 != id2 {
...
func TestClusterID_OrderIndependent(t *testing.T) {
	a := clusterID([]string{vault.NanikaInbox + "/a.md", vault.NanikaInbox + "/b.md", vault.NanikaInbox + "/c.md"})
	b := clusterID([]string{vault.NanikaInbox + "/c.md", vault.NanikaInbox + "/a.md", vault.NanikaInbox + "/b.md"})
	if a != b {
...
func TestClusterID_DifferentPaths(t *testing.T) {
	a := clusterID([]string{vault.NanikaInbox + "/a.md", vault.NanikaInbox + "/b.md", vault.NanikaInbox + "/c.md"})
	b := clusterID([]string{vault.NanikaInbox + "/x.md", vault.NanikaInbox + "/y.md", vault.NanikaInbox + "/z.md"})
	if a == b {
...
func TestClusterID_Format(t *testing.T) {
	id := clusterID([]string{vault.NanikaInbox + "/a.md"})
	if len(id) != 16 {
...
func TestDetectSemanticClusters_FormsSingleCluster(t *testing.T) {
	vec := []float32{1, 0, 0}
	notes := []*promoteNoteInfo{
		{Path: vault.NanikaInbox + "/a.md", Embedding: vec},
		{Path: vault.NanikaInbox + "/b.md", Embedding: vec},
		{Path: vault.NanikaInbox + "/c.md", Embedding: vec},
		// Orthogonal — should not join the cluster.
		{Path: vault.NanikaInbox + "/d.md", Embedding: []float32{0, 1, 0}},
	}
...
func TestLoadUntaggedCaptures_IncludesInboxNotes(t *testing.T) {
	vaultDir := t.TempDir()
	inboxDir := filepath.Join(vaultDir, vault.NanikaInbox)
	...
	clusters := []DetectedCluster{
		{
			...
			MemberPaths:   []string{vault.NanikaInbox + "/a.md", vault.NanikaInbox + "/b.md", vault.NanikaInbox + "/c.md"},
			AvgSimilarity: 0.85,
			Size:          3,
		},
	}
