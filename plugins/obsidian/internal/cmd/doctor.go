...
// Pass a vault.Kind as the optional last argument; omit for KindNanika (backward compat).
func DoctorCmd(jsonOutput bool, kinds ...vault.VaultKind) error {
	...
	if err != nil {
		...
		// 5. Check vault path (kind-aware: second-brain reads its own config + env var)
		var vaultPath, pathLabel, missingHint string
		switch kind {
		case vault.KindSecondBrain:
			vaultPath = cfg.SecondBrainPath
			if vaultPath == "" {
				vaultPath = os.Getenv("OBSIDIAN_SECOND_BRAIN_PATH")
			}
			pathLabel = "Second-brain path"
			missingHint = "Not set. Set second_brain_path in ~/.obsidian/config or OBSIDIAN_SECOND_BRAIN_PATH"
		default:
			...
			missingHint = "Not set. Run 'obsidian configure'"
		}
		...
		if vaultPath != "" {
			checks = append(checks, checkInboxStatus(vaultPath, kind))
		}
	}
...
func checkInboxStatus(vaultPath string, kind vault.VaultKind) DoctorCheck {
	...
	if _, err := os.Stat(inboxPath); os.IsNotExist(err) {
		return DoctorCheck{
			Name:    schema.Inbox,
			Status:  "ok",
			Message: fmt.Sprintf("no inbox folder (%s/ not found)", schema.Inbox),
		}
	}
