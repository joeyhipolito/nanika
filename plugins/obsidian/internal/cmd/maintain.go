...
func MaintainCmd(vaultPath string, staleDays int, fix, jsonOutput bool, kinds ...vault.VaultKind) error {
	...
	inboxDir := filepath.Join(vaultPath, schema.Inbox)
	if _, err := os.Stat(inboxDir); err == nil {
		if inboxNotes, err := vault.ListNotes(vaultPath, schema.Inbox); err == nil {
