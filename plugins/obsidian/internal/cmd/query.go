...
func QueryCmd(vaultPath string, subcommand string, jsonOutput bool) error {
	switch subcommand {
	case "status":
		return HealthCmd(vaultPath, jsonOutput)
	case "items":
		return obsidianQueryItems(vaultPath, jsonOutput)
	case "actions":
...
func obsidianQueryItems(vaultPath string, jsonOutput bool) error {
