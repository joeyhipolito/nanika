...
function ChannelStatusIcons({ channels }: { channels: ChannelStatus[] | null }) {
  ...
  const online = (ch: ChannelStatus | undefined) =>
    !!(ch?.configured && ch?.active && ch?.error_count === 0)

  const iconStyle = (ch: ChannelStatus | undefined, brandColor: string, hovered: boolean): React.CSSProperties => {
    const isOnline = online(ch)
    return {
      display: 'flex',
      color: isOnline || hovered ? brandColor : '#FFFFFF',
      opacity: hovered ? 1.0 : isOnline ? 0.6 : 0.3,
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
      <span
        title={`Discord: ${online(discord) ? 'connected' : 'disconnected'}`}
        style={iconStyle(discord, '#5865F2', hoveredDiscord)}
      ...
      <span
        title={`Telegram: ${online(telegram) ? 'connected' : 'disconnected'}`}
        style={iconStyle(telegram, '#26A5E4', hoveredTelegram)}
