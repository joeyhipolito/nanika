...
// TestParseUserMessage_ToolResultStringContent covers the production wire
// format Claude emits after a tool call: a `user`-typed message whose
// `message.content` is a list with a single `tool_result` block carrying a
// plain-string output. Before TRK-672 fix, parseMessageBestEffort treated
// these as GenericMessage and extractEvents dropped them entirely.
func TestParseUserMessage_ToolResultStringContent(t *testing.T) {
	raw := `{"type":"user","message":{"role":"user","content":[` +
		`{"type":"tool_result","tool_use_id":"toolu_01ABC","content":"hello world","is_error":false}` +
		`]}}`
	msg := parseMessageBestEffort([]byte(raw))
	um, ok := msg.(*UserMessage)
	if !ok {
		t.Fatalf("want *UserMessage, got %T", msg)
	}
	events := extractEvents(um)
	if len(events) != 1 {
		t.Fatalf("want 1 event, got %d", len(events))
	}
	ev := events[0]
	if ev.Kind != KindToolResult {
		t.Errorf("want KindToolResult, got %q", ev.Kind)
	}
	if ev.ToolID != "toolu_01ABC" {
		t.Errorf("want ToolID %q, got %q", "toolu_01ABC", ev.ToolID)
	}
	if ev.ToolOutput != "hello world" {
		t.Errorf("want ToolOutput %q, got %q", "hello world", ev.ToolOutput)
	}
	if ev.ToolIsError {
		t.Error("want ToolIsError=false")
	}
}

// TestParseUserMessage_ToolResultArrayContent covers the alternative wire
// shape where `content` is an array of inner blocks (used for image results
// or text+image multi-block returns). The array form must flatten to a
// single string output without crashing.
func TestParseUserMessage_ToolResultArrayContent(t *testing.T) {
	raw := `{"type":"user","message":{"role":"user","content":[` +
	...
	if ev.ToolID != "toolu_02" {
		t.Errorf("want ToolID %q, got %q", "toolu_02", ev.ToolID)
	}
	if ev.ToolOutput != "line one\nline two" {
		t.Errorf("want flattened ToolOutput, got %q", ev.ToolOutput)
	}
	if !ev.ToolIsError {
		t.Error("want ToolIsError=true")
	}
}

// TestParseUserMessage_NonToolResultIgnored verifies that a user message
// whose content blocks are not tool_result (e.g. plain user text) yields no
// events — the SDK only surfaces tool plumbing on this path.
func TestParseUserMessage_NonToolResultIgnored(t *testing.T) {
	um := &UserMessage{
	...
	if got := extractEvents(um); len(got) != 0 {
		t.Errorf("want 0 events for non-tool_result blocks, got %d", len(got))
	}
}
