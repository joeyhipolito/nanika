...
// When passthrough is true, the full os.Environ() is returned (AllowedEnvVars are already
// included since every key is forwarded); when false, the strict base allowlist is applied.
func filteredEnv(allowed []string, passthrough bool) []string {
	if passthrough {
...
// as a Go duration string (e.g. "10m", "300s"). Defaults to 15 minutes.
