...
// TestStallThresholdDefault verifies the default is 15 minutes when no env var is set.
func TestStallThresholdDefault(t *testing.T) {
	t.Setenv("ORCHESTRATOR_STALL_TIMEOUT", "")
	got := stallThreshold()
	if got != 15*time.Minute {
		t.Errorf("want 15m, got %s", got)
	}
}
...
func TestStallThresholdInvalidEnvVar(t *testing.T) {
	...
	got := stallThreshold()
	if got != 15*time.Minute {
		t.Errorf("want 15m default on bad env var, got %s", got)
	}
}
...
func TestStallThresholdZeroEnvVar(t *testing.T) {
	...
	got := stallThreshold()
	if got != 15*time.Minute {
		t.Errorf("want 15m default on zero duration, got %s", got)
	}
}
...
// TestStartReturnsErrConflictingResumeFlags verifies that Start propagates
// ErrConflictingResumeFlags instead of silently retrying without the resume ID.
func TestStartReturnsErrConflictingResumeFlags(t *testing.T) {
	tests := []struct {
		name string
		opts *AgentOptions
	}{
		{
			name: "both ResumeSessionID and ContinueConversation set",
			opts: &AgentOptions{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := tr.Start(context.Background(), tt.opts)
			if err != ErrConflictingResumeFlags {
				t.Errorf("Start() error = %v, want ErrConflictingResumeFlags", err)
			}
		})
	}
}
