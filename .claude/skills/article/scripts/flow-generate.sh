...
# agent-browser open waits for a post-navigation ready signal (networkidle)
# that never fires on Flow project pages because Flow holds a persistent
# websocket and long-polls the project. Navigation itself completes reliably;
# the non-zero exit is a benign wait timeout. We ignore the exit code and
# verify navigation succeeded via agent-browser get url.
agent-browser open "https://labs.google/fx/tools/flow/project/$PROJECT_ID" >/dev/null 2>&1 || true
for _i in 1 2 3 4 5 6 7 8; do
  _cur_url=$(agent-browser get url 2>/dev/null || echo "")
  if [[ "$_cur_url" == *"/project/$PROJECT_ID"* ]]; then break; fi
  sleep 1
done
if [[ "$_cur_url" != *"/project/$PROJECT_ID"* ]]; then
  echo "ERROR: navigation to Flow project failed — current URL: $_cur_url" >&2
  exit 1
fi
sleep 2
...
# Flow's projectContents.media API returns newest-first. When a tag has been
# generated more than once in this project, iterating last-wins over the map
# file picks the OLDEST entry. Use first-wins semantics instead so we always
# download the most recent generation for each tag.
while IFS='|' read -r tag uuid; do
  [[ -z "$tag" || "$tag" == "unknown" ]] && continue
  [[ -z "${TAG_TO_UUID[$tag]:-}" ]] && TAG_TO_UUID[$tag]="$uuid"
done < "$MAP_FILE"
